...
test('Representative role management and status updates', {
  tag: [
    '@mda',                    // Application tag
    '@representatives',        // Feature tag  
    '@regression',            // Test type tag
    '@[4141]'                 // Azure Test Plans Test Case ID
  ]
}, async ({ page }) => {
  // Your existing test logic - keeping exactly as is for now
  await page.goto(testConfig.mdaUrl);
  await page.getByRole('treeitem', { name: 'Home' }).locator('div').nth(3).click();
  await page.getByText('Representatives').click();
  await page.getByRole('link', { name: 'Shripriya Poomandalam' }).first().click();
...
test('4143', {
  tag: [
    ...
    '@representatives',        // Feature tag  
    '@regression',            // Test type tag
    '@[4143]'                 // Azure Test Plans Test Case ID
  ]
},
  async ({ page }) => {
await page.goto(testConfig.mdaUrl);
